...
const state = {
  ...
  sessionStart: null,      // moment où j'ai ouvert la liste (évite de réannoncer un ancien cadeau)
  toastedGifts: new Set(), // cadeaux déjà annoncés cette session (jamais deux fois)
  selectedUid: null,
  unsubs: [],
...
function stopList(saveSeen = true) {
  ...
  state.since = null; state.sessionStart = null; state.toastedGifts = new Set();
  state.selectedUid = null; state.thread = null; state.nameSynced = false;
}
...
function openList(id) {
  ...
  state.sessionStart = Date.now();  // sert à ne jamais réannoncer un cadeau déjà présent à l'ouverture
...
// Firestore renvoie parfois deux fois la liste complète à l'ouverture (une fois depuis
// le cache de l'appareil, une fois confirmée par le serveur), et marque chaque fois tous
// les cadeaux comme « ajoutés ». On ne se fie donc jamais à ce marqueur seul : un cadeau
// n'est annoncé que s'il a été créé après l'ouverture de la liste, et jamais deux fois.
function announceNewGifts(changes) {
  const prefs = myNotifyPrefs();
  for (const ch of changes) {
    if (ch.type !== "added") continue;
    const id = ch.doc.id;
    if (state.toastedGifts.has(id)) continue;
    state.toastedGifts.add(id);
    const g = ch.doc.data();
    if (ms(g.createdAt, Infinity) <= state.sessionStart) continue;   // déjà là avant que j'ouvre la liste
