...
function stopList(saveSeen = true) {
  ...
  state.since = null; state.selectedUid = null; state.thread = null;
}
...
function ideaCard(n) {
  const img = safeImage(n.imageUrl);
...
function openGiftModal({ item = null, kind = "gift", ownerUid = myUid() } = {}) {
  ...
  let fileImage = item && String(item.imageUrl || "").startsWith("data:image/") ? item.imageUrl : "";
  const startUrl = item && !fileImage ? item.imageUrl || "" : "";
...
function openAccountModal() {
  openModal(`
    ...
    m.querySelector("form").addEventListener("submit", async (e) => {
      ...
      if (name === myName()) return closeModal();
      e.submitter && (e.submitter.disabled = true);
      try {
